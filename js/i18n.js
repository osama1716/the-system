...
(function (SYS) {
  ...
  const STRINGS = {
    ...
    "timer.notLogged": { en: "A clock only — this habit is counted in {unit}, so the time is not added to it.", ar: "مؤقت فقط — هاي العادة محسوبة بالـ{unit}، فالوقت ما بينضاف عليها.", es: "Solo un reloj: este hábito se cuenta en {unit}, así que el tiempo no se le suma.", fr: "Une horloge seulement : cette habitude se compte en {unit}, le temps ne lui est donc pas ajouté.", de: "Nur eine Uhr — diese Gewohnheit wird in {unit} gezählt, die Zeit wird ihr nicht hinzugefügt.", ja: "時計だけです。この習慣は{unit}で数えるため、時間は加算されません。", zh: "只是计时器——这个习惯以{unit}计量，时间不会加进去。" },
    "timer.timeUp": { en: "Time's up — {title}", ar: "خلص الوقت — {title}", es: "Se acabó el tiempo — {title}", fr: "Temps écoulé — {title}", de: "Zeit ist um — {title}", ja: "時間になりました — {title}", zh: "时间到 — {title}" },
