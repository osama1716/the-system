...
(function (SYS) {
  ...
  const STRINGS = {
    ...
    "settings.rulesFixed": { en: "A task's value is its EXP. 100 EXP is one level, and every level grants {n} skill points, placed automatically where the work built them. These are the same for everyone and can't be changed.", ar: "قيمة المهمة هي نقاط خبرتها. كل 100 نقطة خبرة تساوي مستوى واحدًا، وكل مستوى يمنح {n} نقطتي قوة توضعان تلقائيًا حيث بناهما عملك. هذه القواعد واحدة للجميع ولا يمكن تغييرها.", es: "El valor de una tarea es su EXP. 100 EXP equivalen a un nivel, y cada nivel otorga {n} puntos de habilidad, asignados automáticamente donde el trabajo los construyó. Son iguales para todos y no se pueden cambiar.", fr: "La valeur d'une tâche est son EXP. 100 EXP font un niveau, et chaque niveau accorde {n} points de compétence, placés automatiquement là où le travail les a bâtis. Ces règles sont les mêmes pour tous et ne peuvent pas être modifiées.", de: "Der Wert einer Aufgabe ist ihre EXP. 100 EXP ergeben ein Level, und jedes Level gewährt {n} Fertigkeitspunkte, automatisch dort verteilt, wo die Arbeit sie aufgebaut hat. Für alle gleich und nicht änderbar.", ja: "タスクの価値がそのままEXPです。100 EXPで1レベル、各レベルで{n}スキルポイントが与えられ、その働きが伸ばした場所へ自動的に配分されます。全員共通で変更できません。", zh: "任务的价值就是它的经验值。每 100 点经验为一级，每级授予 {n} 点技能点，自动分配到你的努力所构建的位置。所有人一致，不可更改。" },
